...
export const TransactPage = (props) => {
  ...
  const findAccount = (e) => {
      if(e.key === 'Enter'){
          ...
          if(e.currentTarget.id === "account_name") {
      ...
      <main>
        ...
        <section className="content">
          <div className="content-container">
              <form id="transact_form" onSubmit={transact}>
                <h1 className="transact_page">{transactionType}</h1>
                <span className={`notif ${transactionStatus.style}`}>{transactionStatus.msg}</span>
                <div className="row">
                  <div className="column">
                    <label className="transact_labels">Account Name</label>
                    <input 
                        type="text"
                        className="transact_inputs"
                        id="account_name"
                        name="account_name"
                  ...
                  <div className="column">
                    <label className="transact_labels">Account Number</label>
                    <input
                        type="number" 
                        className="transact_inputs"
                        id="account_number" 
                    ...
                    <span className="account_errorNotif" hidden={isAccountNumberFound}>
                        The account name and account number provided
                        do not match any existing account in our records.
                    </span>
                  </div>
                </div>

                <div className="transact_amount">
                  <label className="transact_labels">Current balance</label>
                  <input 
                      type="text"
                      className="transact_inputs"
                      id="current_balance" 
                ...
                <div className="transact_amount">
                  <label className="transact_labels">Amount to {transactionType}</label>
                  <input
                      type="number" 
                      className="transact_inputs"
                      id="input_amount" 
                ...
                <div className="debit_button">
                  <button type="submit" className="transact_btn">{transactionType}</button>
                </div>  
              </form>
          </div>
        </section>
